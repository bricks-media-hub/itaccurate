        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {placements.slice(0, visibleCount).map((placement, index) => (
            <motion.div
              ...
              transition={{ duration: 0.4, delay: index * 0.1 }}
              whileHover={{ scale: 1.03 }}
              className={` flex flex-col justify-evenly h-52 bg-gradient-to-br ${placement.color} rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300`}
            >
              {/* Blur Overlay */}
              <div className="absolute inset-0 bg-white/60 backdrop-blur-md border border-white/60 rounded-xl z-10 pointer-events-none" />
