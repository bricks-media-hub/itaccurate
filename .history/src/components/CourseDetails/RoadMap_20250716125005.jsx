...
/**
 * Horizontal Tree Roadmap Component
 * Usage: <RoadMap data={roadmapData.roadMap} title={roadmapData.whatIs.name} />
 */
export default function RoadMap({ data, title }) {
  const [activeIndex, setActiveIndex] = useState(null);

  return (
    <section className="w-full py-12 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="relative overflow-x-auto scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200">
          <div className="flex items-start space-x-12 py-4">
            {data.map((node, idx) => (
              <Node
                key={idx}
                node={node}
...
function Node({ node, isLast, onActivate, onDeactivate, isActive }) {
  return (
    <div className="relative flex flex-col items-center text-center">
      {/* Connector Bar */}
      {!isLast && (
        <div className="absolute top-7 right-[-6rem] w-24 h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500"></div>
      )}

      {/* Icon Circle */}
      <motion.div
        whileHover={{ scale: 1.2 }}
        className="relative z-10 flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 shadow-lg cursor-pointer"
        onMouseEnter={onActivate}
        onMouseLeave={onDeactivate}
      >
        <img src={node.icon} alt={node.title} className="w-8 h-8" />
      </motion.div>
      ...
      <AnimatePresence>
        {isActive && (
          <motion.div
            ...
            className="absolute top-[5rem] w-64 bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 text-left z-20"
          >
