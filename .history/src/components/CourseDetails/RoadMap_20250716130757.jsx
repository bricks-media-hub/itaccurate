 ...
 * Pinpoint markers on a "road" with gradient connectors
 */
  ...
  return (
    <section className="w-full py-16 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4">
        {/* Title */}
        <div className="text-center mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600 inline-block">
            Roadmap to Learn <span className="text-gray-900 dark:text-white">{title}</span>
          </h2>
        </div>
        ...
        <div className="relative">
          {/* The road line */}
          <div className="hidden md:block absolute inset-x-0 top-10 h-1 bg-gray-300 dark:bg-gray-700"></div>

          {/* Desktop: flex nodes on the road */}
...
function Node({ node, onActivate, onDeactivate, isActive }) {
  return (
    <div className="relative flex flex-col items-center text-center">
      ...
      >
        {/* Map-pin icon */}
        <HiLocationMarker size={48} className="text-blue-500 dark:text-purple-400" />
        {/* Inner circle for your custom icon */}
        <div className="absolute top-1 left-1 w-10 h-10 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-md">
          <img src={node.icon} alt={node.title} className="w-6 h-6" />
        </div>
      </motion.div>

      {/* Title */}
      <h3 className="mt-4 text-lg sm:text-xl font-semibold text-gray-800 dark:text-gray-200">
        {node.title}
      </h3>
      ...
      <AnimatePresence>
        {isActive && (
          <motion.div
            ...
            className="absolute top-[6rem] w-64 bg-white dark:bg-gray-800 rounded-lg shadow-xl p-5 text-left z-20"
          >
