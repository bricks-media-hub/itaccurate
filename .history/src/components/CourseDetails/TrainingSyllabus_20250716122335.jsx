...
const TrainingSyllabus = ({ syllabus }) => {
    ...
    // Use useRef to store colors persistently without triggering re-renders
    const sectionColors = useRef([]);

    // Filter out the first item if needed (assuming it's a header)
    const filteredData = syllabus.slice(1);

    // Initialize colors only once when component mounts
    useEffect(() => {
        if (sectionColors.current.length === 0 && filteredData.length > 0) {
            sectionColors.current = filteredData.map(() => {
                return defaultColors[Math.floor(Math.random() * defaultColors.length)];
            });
        }
    }, [filteredData]);

    // Get color for section - uses the pre-assigned random color
    const getSectionColor = (index) => {
        return sectionColors.current[index] || DEFAULT_COLOR;
    };
                ...
                <div className="flex flex-col lg:flex-row gap-8 justify-center">
                    ...
                    <div className="lg:w-2/3">
                        {filteredData.length > 0 && (
                            ...
                            >
        <div className={`p-6 flex items-center gap-4 ${getSectionColor(activeSection)} text-white`}>                                    <motion.div
                                        animate={{ rotate: 360 }}
