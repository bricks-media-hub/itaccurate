...
const ConnectionsPipes = ({ roadmapData, hasOnlyOneRow }) => (
  <div className="absolute left-0 right-0 top-0 bottom-0 pointer-events-none hidden md:block z-0 overflow-hidden">
    <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
      ...
      {/* Main horizontal pipe with animation and glow */}
      {!hasOnlyOneRow && (
        <motion.path
          d="M10,50 L90,50"
      ...
      {roadmapData.roadMap.map((_, index) => {
        ...
        return (
          <g key={index}>
            {/* Animated connection pipe with growing animation */}
            <motion.path
            ...
            {/* Subtle ripple effect around dots */}
            <motion.circle
              cx={x}
              cy={yStart}
              r="1.5"
