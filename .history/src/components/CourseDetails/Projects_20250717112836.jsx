          ...
          <Slider {...settings}>
            {filteredProjects.map((project, index) => (
              <div key={index} className="px-3 py-2 h-full focus:outline-none">
                <div className="relative rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 h-[420px] group">
                    ...
                    <div className="transform transition-transform duration-300 group-hover:-translate-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {project.technologies?.map((tech, i) => (
                          <span 
                            key={i}
                            className="text-xs font-medium text-blue-100 bg-blue-900/40 px-2.5 py-1 rounded-full"
                          >
                            {tech}
                          </span>
                        ))}
                      ...
                      {project.link && (
                        <a 
                          href={project.link} 
                          className="mt-4 inline-block text-sm font-medium text-white hover:text-blue-200 transition-colors"
                        >
                          View Project →
                        </a>
                      )}
