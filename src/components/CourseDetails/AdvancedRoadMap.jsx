...
export default function AdvancedRoadMap() {
  ...
  const desktopRows = [];
  for (let i = 0; i < roadmapData.roadMap.length; i += itemsPerRow) {
    desktopRows.push(roadmapData.roadMap.slice(i, i + itemsPerRow));
  }
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        {isMobile ? (
          ...
          <div ref={roadContainerRef} className="relative">
            <EnhancedHighwayDesktop />
            {roadWidth > 0 && <CyclingBoyDesktop roadWidth={roadWidth} />}

            {desktopRows.map((rowItems, rowIndex) => (
              <div
                key={rowIndex}
                className={`grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 mt-${rowIndex === 0 ? '0' : '32'} relative`}
              >
                {rowItems.map((mod, i) => (
                  <RoadmapItem
                    key={rowIndex * itemsPerRow + i}
                    module={mod}
                    index={rowIndex * itemsPerRow + i}
                    onClick={() => setSelectedModule(mod)}
                    position={rowIndex % 2 === 0 ? 'top' : 'bottom'}
            ...
            {connectionsVisible && desktopRows.length > 1 && (
              <BezierConnectorsDynamic rowCount={desktopRows.length} />
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div className="absolute inset-x-0 top-[55%] h-28 -mt-14 z-0 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-0 right-0 h-1 transform -translate-y-1/2"
        style={{ background: "repeating-linear-gradient(90deg, transparent, transparent 20px, white 20px, white 40px)" }}></div>
...
function CyclingBoyDesktop({ roadWidth }) {
  return (
    ...
    >
      <img src="/icons/bike-rider.svg" alt="Cycling Boy" className="w-full h-full object-contain" />
    </motion.div>
  );
}

// Dynamic bezier connectors between rows
function BezierConnectorsDynamic({ rowCount }) {
  const total = 4;
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none z-0" preserveAspectRatio="none">
      {Array.from({ length: rowCount - 1 }).map((_, rowIndex) =>
        [...Array(total)].map((_, i) => {
          ...
          const startY = 20 + rowIndex * 30;
          const midY = startY + 15;
          const endY = startY + 30;

          return (
            <g key={`conn-${rowIndex}-${i}`}>
              <path
                d={`M${startX}%,${startY}% C${startX}%,${midY}% ${(startX + 4)}%,${midY}% ${(startX + 4)}%,${endY}%`}
                stroke="#4a5568"
                strokeWidth="8"
                fill="none"
                strokeLinecap="round"
...
//   const cardRadius = baseRadius + minDim * 0.15;
...
//           {/* Connecting lines */}
//           {width > 0 && roadmapData.roadMap.map((_, i) => {
...
//           {/* Module cards outside the circle */}
//           {width > 0 && roadmapData.roadMap.map((mod, i) => {
