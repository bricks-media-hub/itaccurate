...
const Footer = ({ activeLocation, setActiveLocation }) => {
  ...
  const contactInfo = [
    ...
    {
      icon: (
        <img src="/icons/map-pin-icon.png" alt="mail" className="w-5 h-5" />
      ),
      location: "Nagpur",
      mapId: "nagpur",
      address:
        "607, 608 B-wing, Lokmat Bhavan, Lokmat Square, Ramdaspehi, Nagpur Ph. - ",
      phone: "09175978889",
    ...
    },
    {
      ...
      mapId: "thane",
      address:
        "Office No. 806, 8th Floor, Paradise Tower, next to McDonald's, Naupada, Thane West Ph. - ",
      phone: "07738277389",
    ...
    <footer className="bg-gray-50 dark:bg-black text-gray-800 dark:text-gray-200 pt-12 pb-6 px-4 sm:px-6 lg:px-8 shadow-lg transition-colors duration-300">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wide">
            IT ACCURATE
          </h2>
          ...
          <ul className="space-y-2">
            {legalLinks.map((link, index) => (
              <li key={index}>
                <Link
                  to={link.to}
                  className="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors duration-200 flex items-center group"
                >
                  <span className="group-hover:translate-x-1 transition-transform duration-200">
                    {link.label}
                  </span>
