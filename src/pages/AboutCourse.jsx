...
function AboutCourse() {
  const { courseDetail, seoData } = useLoaderData();
  const location = useLocation();
  // console.log(courseDetail);
  // console.log(seoData);

  const { title, description, keywords, canonical, scriptTag } = {
    ...DEFAULT_SEO,
    ...seoData,
  };
    ...
    <>
      ...
      <div className="relative bg-white dark:bg-gray-900 transition-colors duration-300">
        ...
        {courseDetail.roadMap && (
          <RoadMap roadmapData={courseDetail.roadMap} name={courseDetail?.whatIs?.name}/>
        )}
